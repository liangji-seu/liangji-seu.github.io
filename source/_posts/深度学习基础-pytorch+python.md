...
那如何来生成训练数据？
> 因为我们不能指望依赖静止的数据，预测新的变化，因为新的动力学本身一定收到新的数据的影响
...
所以，这个式子，左边里面的x是实际发送的序列，是**观测**，而我们的自回归这些序列模型，是期望得到**观测的概率**，也就是预测，所以我们的序列模型要实现的是一个概率模型P(x)，他得到的是预测（对观测的一个概率分布）

触及了**概率图模型**和深度学习**序列建模**的核心本质

1. 观测与概率的关系
- **左边的 $P(x_1, \dots, x_T)$**：确实代表了整个**实际观测序列**在真实世界中发生的概率分布。
- **模型的任务**：我们的自回归模型（如 TCN、LSTM）本质上就是在<mark style="background:#fff88f">尝试拟合（Approximating）这个真实分布</mark>。我们希望模型学到的分布 $P_{model}$ 尽可能接近真实观测的分布 $P_{real}$。

1. 预测即概率分布
- **预测的本质**：正如你所说，<mark style="background:#fff88f">序列模型</mark>实现的确实是<mark style="background:#ff4d4f">一个概率模型</mark>。当我们说“预测下一个值”时，模型输出的其实不是一个单一的绝对数值，而是在给定历史条件下，下一个可能观测值的**概率分布**。
- **回归 vs 分类**：
    - 在离散场景（如 NLP）中，它是所有单词的概率表。
    - 在连续场景（如你研究的**外骨骼扭矩预测**）中，模型虽然通常输出一个具体数值 $\hat{x}_t$，但从数学上看，这相当于假设概率分布是一个以 $\hat{x}_t$ 为均值的高斯分布。


所以

**链式法则的意义**：公式 (8.1.2) 将复杂的全局概率 $P(x_1, \dots, x_T)$ 拆解为一个个可计算的局部步步预测 $P(x_t \mid x_{t-1}, \dots, x_1)$。


**期望得到预测**：通过最大化观测序列在模型下的出现概率（即最大似然估计 MLE），我们强迫模型在给定历史 $(x_{t-1}, \dots, x_1)$ 时，能给真实的 $x_t$ 分配最高的概率。这样，模型就具备了“预测”能力



两个问题：
<mark style="background:#affad1">既然我们的序列模型，是为了学习数据的概率分布规律，是这么理解吧，但是模型最终会输出一个具体的预测值啊，并不是输出完整的分布规律比如期望，均值这些。那他输出的内容和这个随机变量属于的分布律是什么关系呢</mark>

虽然模型只给你一个数值，但这个数值背后隐藏着一整个分布律：
...
**损失函数定义了“惩罚”规则，而模型为了躲避惩罚，被迫学会了输出该分布下的特定统计量。**

对于回归模型来说，为什么输出是期望值（均值）

<mark style="background:#fff88f">1. 回归模型：为什么输出的是“期望值（均值）”？</mark>

当你训练 TCN 预测外骨骼扭矩并使用 **L2 Loss（均方误差 MSE）** 时：
- **惩罚机制**：MSE 的计算方式是 $Loss = (y - \hat{y})^2$。
- **数学逼近**：从概率统计的角度看，使 MSE 最小化的最优预测值 $\hat{y}$ 正好就是条件概率分布的**均值（期望）**。
    - <mark style="background:#b1ffff">_原理_：如果你有一个分布，你想找一个点，让所有样本到这个点的距离平方和最小，这个点在数学上证明必然是均值</mark>。
- **例外情况**：如果你改用 **L1 Loss（平均绝对误差 MAE）**，即 $Loss = |y - \hat{y}|$，模型为了最小化损失，会学会输出分布的**中位数（Median）**。

**结论**：通过使用 MSE 损失函数，你就在数学上“命令”模型去寻找并输出分布的期望值。


对于分类模型来说，为什么输出的是概率最大的值

<mark style="background:#fff88f">2. 分类模型：为什么输出的是“概率最大的那一个”？</mark>
当你做动作识别并使用 **交叉熵损失（Cross-Entropy Loss）** 时：
- **惩罚机制**：交叉熵会衡量模型输出的概率分布与真实标签（One-hot 编码）之间的差异。
- **数学逼近**：交叉熵的本质是最大似然估计（MLE）。它会迫使模型在正确的类别上分配尽可能高的概率。
- **推理决策**：在推理阶段，虽然 Softmax 层给出了所有类别的概率分布，但我们通常通过 `argmax()` 操作提取概率最高的索引。
- **数学本质**：这对应于离散分布中的**众数（Mode）**。模型被训练成给最可能发生的事件分配最高权重。

---

所以，假设我们面对回归问题，选取不同的损失函数，实际上，是影响模型输出取的分布的估计量是吗

所以我总结以下就是：

我们在回归模型里面，我们的模型学会的是条件概率分布，他会让输出满足一个条件概率分布，（条件就是输入），所以我们的模型就是一个条件概率模型。

  

训练数据集，就是实际蕴含分布律规律的采样点。

  

损失函数，其实就是关于我们如何来从这个模型的条件概率分布率，给定条件的分布率下，得到一个具体的值，也就是该条件概率分布下的估计量


<mark style="background:#affad1">gemini的答案：</mark>
1. **模型**即**条件概率模型**：$P(y|x)$
在回归问题中，给定输入 $x$（如你的 IMU 传感器数据），模型不再只是一个简单的函数 $f(x)$，它本质上是在建模 **$P(y|x)$**

**输入 $x$ 是“条件”**：它限定了当前系统所处的上下文（Context）

**输出满足分布**：对于同一个输入 $x$，真实世界中的 $y$（扭矩）可能因为噪声或未观测变量（如肌肉疲劳）而存在波动，这个波动形成的规律就是条件概率分布

**模型是“参数化分布”**：你的 TCN 网络其实就是这个概率模型的参数化表达，它学习如何根据不同的 $x$ 去调整分布的形态。


2. **训练集即“样本采样”**
- **数据集是观测点**：每一组 $(x, y)$ 都是从那个未知的、真实的联合分布 $P(x, y)$ 中进行的 一次**随机采样**。
- **训练的目标**：我们手头只有离散的采样点，但我们想通过这些点去反推（拟合）出那个连续的条件概率分布 $P(y|x)$。


 2. 损失函数即“估计量提取器”
这是最核心的一点。既然模型学到了一个完整的分布，但它最后只能吐出一个具体的标量值，**损失函数就决定了这个值的“身份”**：
- **MSE (均方误差)**：强制模型输出该分布的**期望（均值）**。这在物理上代表了“最小能量”或“平均功率”意义上的最优估计。
...
所以，<mark style="background:#affad1">我想再总结以下，</mark>

> 我目前从最开始的线性回归单元，到MLP，到MLP+softmax, 到CNN卷积，到现在的RNN序列模型
>
> 除了带softmax的那个是分类模型，其他都是回归模型是吧。
>
>
> 比如MLP，回归，学习的是（X ,Y ）的分布律，然后MLP（x）从联合分布律中得到条件分布率，然后指定使用的损失函数MSE训练，来设置模型的输出是选择的条件分布率的均值估计值。
>
> 又比如MLP+softmax分类，学习的是（X，Y）的分布律，也是得到条件分布律，然后用交叉熵损失函数训练，设置模型选择条件分布律中的概率最大的值。
> 
> 又比如CNN+全连接的LeNet 这些，还是回归模型？还是分类模型？还是和上面一样的理解？
> 
>现在的RNN序列模型，本质上，是学习{(xt-1, xt-2, ...., x1), xt(标签真值)}序列的概率分布，然后模型得到输入（xt-1, xt-1, ...., x1）后，就得到了p(xt | xt-1, xt-2, ..., x1)的条件概率分布了。之后损失函数让我们继续从这个条件分布里面得到一个具体的估计量。也就是xt_hat是吗


我们可以顺着你的思路，把这些模型一一对号入座，完善你的理论版图：

<mark style="background:#fff88f"> 1. 回归与分类的界限</mark>

你的理解非常准确。**回归与分类的本质区别不在于中间的卷积或循环层，而在于最后的“输出层”和“损失函数”：**

- **线性回归、MLP、CNN（用于预测数值）**：只要末端输出的是连续值且配合 MSE/MAE 等损失函数，它们都是**回归模型**。
- **MLP + Softmax、CNN + Softmax (如 LeNet)**：只要末端使用了 Softmax 将输出映射为概率分布，并配合交叉熵损失，它们就是**分类模型**。

<mark style="background:#fff88f"> 2. 对 MLP 和分类模型的复盘</mark>

- **MLP 回归**：完全如你所说，它建模 $P(Y|X)$，通过 MSE 引导模型输出条件分布的**均值**。
- **MLP + Softmax 分类**：它建模离散的条件分布 $P(Y|X)$。交叉熵训练使得模型在正确类别上的概率最大化，最终推理时我们取其**众数（概率最大者）**。


<mark style="background:#fff88f"> 3. 如何理解 CNN (如 LeNet)？</mark>

**LeNet 既可以是分类模型，也可以是回归模型，取决于你拿它做什么：**

- **做分类（原始用途）**：LeNet 最后一层接 Softmax 识别数字 0-9。此时它学习的是 $P(数字类别 | 图像像素)$。它依然是得到条件分布，然后选概率最大的。
- **做回归**：如果你把 LeNet 最后的 Softmax 去掉，让它预测图像中物体的坐标（连续值），它就变成了回归模型。逻辑瞬间切换到：得到 $P(坐标 | 图像)$ 的条件分布，通过 MSE 输出**均值估计**。
- **本质逻辑**：CNN 的卷积层只是在做**特征提取**（把高维像素空间映射到低维特征空间），而最后的“全连接层 + 损失函数”才决定了它是如何从条件分布中提取估计量的。
    

<mark style="background:#fff88f">4. 对 RNN/TCN 序列模型的升华</mark>

你对 RNN 的总结非常精辟，这就是自回归（Autoregressive）的精髓：

- **学习目标**：它学习的是序列的联合概率分布 $P(x_1, \dots, x_T)$。
- **条件化**：通过链式法则，模型在每一个步长 $t$，都在计算 $P(x_t \mid x_{t-1}, \dots, x_1)$。
- **估计量提取**：
    - **输入**：历史序列 $(x_{t-1}, \dots, x_1)$。
    - **得到**：当前时刻的条件分布。
...
是的，你的理解完全正确！无论模型多复杂，它都在做两件事：

1. **建模条件分布 $P(Y \mid X)$**。
    
2. **根据损失函数从分布中“抠”出一个最代表性的值作为输出**。

---

最后一个问题

> 那么，还有最后一个问题，对于序列模型，P(x1, x2, ......, xT),这个该叫什么？为什么是要预测一整个序列的概率？
>
>我们普通的回归模型，给定x后，模型得到P(Y|X)的条件概率分布，然后选出一个估计量，作为y的具体值。
>
>但是为什么序列模型，我能理解模型每次推理得到的是P(xt | xt-1, ....., x1), 把这些每次的概率乘起来，是不是就相当于自回归？
>
>还是说，如果我是离线测试，我已经拥有了完整的序列，这个时候还叫自回归嘛？我并没有把自己预测的xt，当作下一次推理的输入，而是把原本数据集里面的xt当作输入，这种离线测试。
>
>还是说自回归，比如要把自己的预测值，重新作为输入？

<mark style="background:#affad1"> 1. P(x1, x2, ..., xT) 叫什么？</mark>

在统计学中，这被称为**联合概率分布（Joint Probability Distribution）**。

- **为什么要预测一整个序列的概率？** 因为序列中的数据不是独立的，前后的数值存在强耦合（动力学规律）。预测整个序列的概率，本质上是在评估“这一串数据作为一个整体，符合真实物理规律的可能性有多大”。
    
- **整体与局部的关系**：
    
    根据链式法则，整个序列的概率被分解为每一步条件概率的乘积：
    
    $$P(x_1, \dots, x_T) = \prod_{t=1}^T P(x_t \mid x_{t-1}, \dots, x_1)$$
    
    。所以，虽然我们每次只算一步 $P(x_t \mid \dots)$，但我们的终极目标是让整条曲线（联合分布）最接近真实情况。

<mark style="background:#affad1">2. 什么是真正的“自回归”？</mark>

你的直觉非常敏锐。在工程实践中，根据你如何处理“预测值”，会演变成两种完全不同的模式：

 <mark style="background:#fff88f">模式 A：Teacher Forcing（你说的“离线测试/训练”）</mark>

在这种模式下，即使模型在 $t$ 时刻预测错了，你在 $t+1$ 时刻依然把数据集里的真值（Ground Truth）喂给模型。

- **是否叫自回归？** 严格来说，这叫**自回归模型的训练/并行推理**。它利用了序列的自回归结构，但没有形成反馈环。
    
- **特点**：计算速度快（可以并行），且不会产生误差累积。你目前在 RTX 4090 上训练 TCN 预测扭矩，大概率就是这种模式。
    

<mark style="background:#fff88f"> 模式 B：Autoregressive Generation（真正的“闭环自回归”）</mark>

当你真正要把预测出的 $\hat{x}_t$ **重新当作输入**喂回给模型，去预测 $\hat{x}_{t+1}$ 时，这才是最纯正的自回归生成。

- **特点**：这是一种迭代过程。模型必须对自己产生的错误负责，误差会随着时间步不断累积（Drift）。
...
#### 马尔可夫模型
我们之前在自回归模型的近似法中，用r序列长度的序列，来代替完整的历史序列{xt-1, ...., x1}

只要这种是近似精确的，我们就说序列**满足马尔可夫条件**
...
- **目标一致**：普通回归预测的是 $P(Y|X)$，而自回归预测的是 $P(x_t | x_{t-1}, \dots, x_{t-\tau})$。由于输出 $x_t$ 是连续的数值（你的正弦波数据），所以它遵循回归任务的所有统计规律。
    
- **损失函数验证**：你使用的 `nn.MSELoss`（均方误差）正是回归任务的标准配置。如我们之前讨论的，它强迫 MLP 输出条件分布的**期望值**。
...
"""
准备数据集：
一个长度为4的序列为特征，下一个值为标签， 用前600个时间步来训练
...
"""取:n_train, 即前600行的（feature,labels）作为训练集， 构造训练集迭代器"""
train_iter = d2l.load_array((features[:n_train], labels[:n_train]),
                            batch_size, is_train=True)
"""
数据集如下
features = [0号,1号,2号,3号]    labels = [4号]
           [1号,2号,3号,4号]             [5号]
            ...                          ...
           [995号,996号,997号,998号]     [999号]
           
features.shape = (996,4)        labels.shape = (996,1)
训练集，从（features, labels）里面选取前600行，batch_size = 16
"""
...
"""
简单的构造一个由全连接构造的回归模型
所以，这个模型仅仅只是实现一个自回归的模型，（RNN是循环卷积，本质上是自回归模型的子类）
"""
...
"""定义损失函数对象，因为是回归模型，预测序列, 返回每个序列样本的损失张量，不计算均值"""
...
            l.sum().backward()
            trainer.step()
        print("epoch %d, loss %4f" % (epoch, d2l.evaluate_loss(net, train_iter, loss)))
...
d2l.plot([time, time[tau:]], # 横轴显示两个量：time(完整1000个数据)[0,1000), 测试用[4,1000)
         # 纵轴显示两个量：x(完整的1000个数据值)[0,1000), 测试用的预测值
...
### QA
>我想知道，在训练阶段，是一口气输入目标序列， 经过掩码多头自注意力后，生成的序列，每个token: 
>1. 知道自己和之前的token的关系（掩码自注意） 
>2. 每个token知道自己更高维的特征（多头） 是吧

>1. 我很好奇，我要如何理解这个掩码多头自注意力的输出序列的数学含义呢？（就是我好奇他训练阶段，一口气输入，然后最终解码器产生的序列也是一口气输出，就是整个输入序列的后一个token的预测是吧。这个后一个，是由掩码来决定的） 
 
>然后这个掩码多头自注意力的输出序列，（里面携带了哪些信息？）输入多头注意力，KV是编码器对源序列的处理结果（该KV序列，蕴含了原始序列的哪些信息？我要如何形象的理解？） 
>
