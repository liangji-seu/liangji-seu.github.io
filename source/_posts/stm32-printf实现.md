...
tags: [嵌入式, mcu, stm32]
---
- [1. 标准库 printf 原理](#1-标准库-printf-原理)
- [2. stm32中的printf实现](#2-stm32中的printf实现)
- [3. 为什么是否使用微库都能成功？](#3-为什么是否使用微库都能成功)
    - [1. 微库（Micro LIB）与标准库的共性：依赖`fputc`作为输出入口](#1-微库micro-lib与标准库的共性依赖fputc作为输出入口)
    - [2. 微库与标准库的差异不影响`fputc`的重写逻辑](#2-微库与标准库的差异不影响fputc的重写逻辑)
    - [3. 重写`fputc`的本质：覆盖默认输出目标](#3-重写fputc的本质覆盖默认输出目标)
    - [总结](#总结)
