...
**以上解释了，我们矩阵乘Wq,Wk,Wv三个算子层的作用，下面来解释为什么**
- q是4096
- k是1024
- v是1024
...
所谓多头，其实就是去切分q,k,v的这么长的向量，不同分块sub就是一个头。

以llama3为例，
- k,v  1024维度，切分8个头，每个头128维
...
所以，一个token向量，投影成QKV向量后，
- kv向量加入kvcache，作为这个token的键值缓存，用来被其他token向量计算相关性。
- Q向量则用来和历史token的KV向量计算相关性。 

 所以，因为**每个向量的Q，K经过RoPE**，这样，这个token和另一个token的QK进行**计算相关性**的时候，就带入了相对位置关系，是吧
...
可以看到，我们**前面的多头策略，本质上就是定义两个不同维度的向量的点积运算**。
...
在输出注意力输出的时候，因为v是1024维度，只有8个头，所以，和k一样，GQA共享，也就是说，每组的q

所以，i个kcache, i个vcache, 一个q 4096, 32个头，

**最终产生的v_q, 就是4096维度的**（32个头都有分数，分组加权vcache）
